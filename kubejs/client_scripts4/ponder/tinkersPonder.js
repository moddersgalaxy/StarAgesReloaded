//Script by GhostyRex
Ponder.tags((nadien) => {
      nadien.createTag("kubejs:smeltery_foundry", "tconstruct:smeltery_controller", "Tinkers Forges", "Creating the Smeltery and Foundry", [
...
Ponder.registry((nadien) => {
    nadien.create("tconstruct:smeltery_controller").scene("smeltery_intro", "Creating the Smeltery", "kubejs:tinkersponder1", (scene, util) => {
	scene.showBasePlate()
    scene.idle(10)
    for (let x = 4; x < 7; x++) {
        for (let z = 3; z < 6; z++) {
    ...
    nadien.create(["tconstruct:seared_faucet","tconstruct:seared_channel","tconstruct:scorched_faucet","tconstruct:scorched_channel"]).scene("faucet_channel", "Pouring Fluids", "kubejs:tinkersponder3", (scene, util) => {
    scene.showStructure()  
    scene.idle(20)
    ...
    nadien.create(["tconstruct:seared_basin","tconstruct:seared_table","tconstruct:scorched_table","tconstruct:scorched_basin"]).scene("basin_table", "Using Basins and Tables", "kubejs:tinkersponder5", (scene, util) => {
    scene.showBasePlate() 
    ...
    nadien.create("tconstruct:foundry_controller").scene("foundry", "Creating the Foundry", "kubejs:tinkersponder6", (scene, util) => {
    scene.showBasePlate() 
    scene.idle(20)
    for (let x = 2; x < 7; x++) {
        for (let z = 2; z < 7; z++) {
    ...
    nadien.create(["tconstruct:seared_chute","tconstruct:scorched_chute","tconstruct:seared_duct","tconstruct:scorched_duct"]).scene("chute_duct", "Chutes and Ducts", "kubejs:tinkersponder8", (scene, util) => {
    scene.showStructure()
    scene.idle(20)
